        ...
        if (node2distances[node] < max_depth):
            for neighbour in graph.neighbors(node):
                if neighbour not in seen:
                    node2distances[neighbour] = node2distances[node] + 1
                    queue.append(neighbour)
                    seen.add(neighbour)
                if node2distances[neighbour] > node2distances[node]:
                    node2parents[neighbour].add(node)
                    node2num_paths[neighbour] = max(node2num_paths[node],node2num_paths[neighbour]) + len(node2parents[neighbour]) - 1
        level = node2distances[node]
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    bottom_up_result = defaultdict(list)
    current_weight = defaultdict(int)
    for node in node2distances.keys():
        current_weight[node] = 1
    sorted_distances = sorted(node2distances.items(), key=lambda x: x[1], reverse=True)
    for node in sorted_distances:
        total_weight_parents = sum([node2num_paths[parent] for parent in node2parents[node[0]]])
        for parent in node2parents[node[0]]:
            couple = (min(node[0],parent), max(node[0],parent))
            bottom_up_result[couple] = float(current_weight[node[0]] * node2num_paths[parent] / total_weight_parents)
            current_weight[parent] += bottom_up_result[couple]
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        ...
        betweenness_temp = bottom_up(node, node2distances, node2num_paths, node2parents)
        betweenness_results = dict(Counter(betweenness_temp)+Counter(betweenness_results))
    betweenness_results.update((x, y/2) for x, y in betweenness_results.items())
...
def partition_girvan_newman(graph, max_depth):
    ...
    sorted_weight = sorted(betweenness.items(), key=lambda x: x[1], reverse=True)
    for edge in sorted_weight:
        newgraph.remove_edge(edge[0][0], edge[0][1])
        partitions = get_components(newgraph)
        if (len(partitions) == 2):
            return partitions
    return partitions
...
def get_subgraph(graph, min_degree):
    ...
    subdegrees = {k: v for k, v in degrees.items() if v >= min_degree}
    return graph.subgraph(subdegrees)
...
def brute_force_norm_cut(graph, max_size):
    ...
    for i in range(max_size):
        ...
        cut_combinations = combinations(new_graph.edges(), i+1)
        for edges in cut_combinations:
            new_graph = graph.copy()
            new_graph.remove_edges_from(edges)
            components = get_components(new_graph)
            if len(components) >= 2:
                scores.append((norm_cut(components[0].nodes(),components[1].nodes(), graph), list(edges)))
    return scores
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        if len(partitions) >= 2:
            cut_value = norm_cut(partitions[0].nodes(), partitions[1].nodes(),graph)
        score.append((depth, cut_value))
    return score
...
def make_training_graph(graph, test_node, n):
    ...
    """
    newgraph = graph.copy()
    for neighbor in (sorted(newgraph.neighbors(test_node)))[:n]:
...
def jaccard(graph, node, k):
    ...
    for candidate in graph.nodes():
        if candidate != node and candidate not in set(graph.neighbors(node)):
            neighbors2 = set(graph.neighbors(candidate))
            neighbors1 = set(graph.neighbors(node))
            scores.append(((node,candidate), len(neighbors2 & neighbors1) /
                              len(neighbors2 | neighbors1)))
...
def evaluate(predicted_edges, graph):
    ...
    0.5
    """
    return float(len(set(graph.edges()).intersection(predicted_edges))/len(predicted_edges))
